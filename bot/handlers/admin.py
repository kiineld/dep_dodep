...
@router.callback_query(F.data == "admin:rw_squads")
async def cb_admin_rw_squads(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("❌ Нет доступа", show_alert=True)
        return
    squads = await remnawave.get_internal_squads()
    if not squads:
        await callback.answer("Нет сквадов или нет связи с Remnawave", show_alert=True)
        return
    text = (
        "🗂 <b>Internal Squads (Remnawave)</b>\n\n"
        "Скопируйте UUID нужного сквада в .env:\n"
        "<code>BASIC_SQUAD_UUID=...</code>\n"
        "<code>WHITELIST_SQUAD_UUID=...</code>\n\n"
    )
    for sq in squads:
        name = sq.get("name", "—")
        uuid = sq.get("uuid", "—")
        members = sq.get("usersCount", sq.get("membersCount", "?"))
        text += f"📁 <b>{name}</b>\n<code>{uuid}</code>\nУчастников: {members}\n\n"
    await _edit_or_send(callback, text, admin_back_keyboard("admin:remnawave"))
    await callback.answer()

...
async def _purchase_subscription(
    ...
    # ── 2. Create or update user in Remnawave ──
    ...
    if remnawave_uuid and (settings.basic_squad_uuid or settings.whitelist_squad_uuid):
        ...
        if squad_assigned:
            logger.info(
                f"User {db_user.id} (uuid={remnawave_uuid}) assigned to "
                f"{'basic+whitelist' if vpn_type == 'whitelist' else 'basic'} squad(s)"
            )
        else:
    ...
    if not squad_assigned and (settings.basic_squad_uuid or settings.whitelist_squad_uuid):
        text += "\n\n⚠️ Не удалось назначить сервер. Обратитесь в поддержку."
